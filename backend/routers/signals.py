...
@router.get("/latest")
def get_latest_signals(session: Session = Depends(get_session)):
    """
    Return the latest AI-generated investment signals based on current mood scores.
    Signals are generated by Claude and stored in the most recent snapshot.
    """
...
async def generate_signals(session: Session = Depends(get_session)):
    """
    Re-run Claude investment signal and tech momentum analysis
